...
**새벽 2시 47분, 당직 폰이 울립니다. "결제 API 5xx 급증, 고객 문의 유입 중."**
Enterprise 서비스 운영이라면 그다음 한 시간이 늘 이렇게 흘러갑니다.

- **원인 후보를 좁히기까지 한 시간이 걸립니다.** 관제 1차 통보 → 담당자 호출 → VPN·Bastion 접속 →
  권한 확인까지가 겨우 시작입니다. 모니터링 · APM · 로그 · 배포 이력이 전부 다른 시스템에 흩어져 있고,
  앱 · DB · 네트워크 · 인프라 담당을 콘퍼런스 콜로 불러 모으는 동안 *"저희 쪽은 이상 없습니다"* 만 세 번 듣습니다.
  그사이 SLA 시계는 계속 돌아가고 고객 문의는 쌓입니다.
- **그때 담당했던 사람이 있어야 풀립니다.** 지난 분기에 똑같은 증상을 겪었지만, 원인과 조치 내역은
  개인 메모나 이미 닫힌 대화방에 흩어져 있습니다. 그 사람이 휴가·조직 이동이면 처음부터 다시 시작합니다.
- **복구가 끝나도 일이 남습니다.** 타임라인 재구성, 장애 보고서, 원인·재발 방지 대책 —
  경영층 보고와 유관 부서 회람까지 포함하면 복구보다 사후 처리에 시간이 더 들어갑니다.

> **Azure SRE Agent 는 이 세 구간을 대신합니다.**
> 알림이 울린 그 순간 에이전트가 먼저 깨어나 로그·메트릭·배포 이력·런북을 한 번에 상관 분석하고,
> 당직자가 접속을 마쳤을 때는 **원인 · 영향 범위 · 다음 조치가 이미 채워진 조사 결과(Investigation)** 가
> 기다리고 있습니다. 조사 내용은 팀 메모리에 남아 **다음 장애 때 자산으로 재사용**됩니다.
