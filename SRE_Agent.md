...
**새벽 3시, 당직 폰이 울립니다. "결제 API 5xx 급증".**
운영 부서라면 그 다음에 벌어지는 일이 늘 똑같습니다.

- **상황 파악에만 30분이 흘러갑니다.** 노트북 열고, VPN 붙고, 모니터링 → APM → 로그 → 최근 배포 이력을
  순서대로 열어보는 사이에 상황실에서는 이미 *"원인이 뭐냐"* 는 질문이 세 번째 들어옵니다.
- **그때 담당했던 사람이 있어야 풀립니다.** 지난 분기에 똑같은 장애를 겪었지만, 그때의 원인과 조치는
  담당자 머릿속이나 어느 대화방에 흩어져 있습니다. 그 사람이 휴가면 처음부터 다시 시작합니다.
- **장애가 끝나도 일이 남습니다.** 타임라인 재구성, 장애 보고서, 재발 방지 대책 —
  복구보다 사후 문서 작업에 시간이 더 들어갑니다.

> **Azure SRE Agent 는 이 세 구간을 대신합니다.**
> 알림이 울린 그 순간 에이전트가 먼저 깨어나 로그·메트릭·배포 이력·런북을 한 번에 상관 분석하고,
> 당직자가 노트북을 열었을 때는 **원인 · 영향 범위 · 다음 조치가 이미 채워진 조사 결과(Investigation)** 가
