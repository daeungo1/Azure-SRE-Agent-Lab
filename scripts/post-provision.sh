...
# Windows compatibility: python3 may be 'python', and the Windows Store alias stub
# named python3 resolves but produces no output — probe each candidate before using it.
PYTHON=""
for candidate in python3 python py; do
  if command -v "$candidate" &>/dev/null && [ "$("$candidate" -c 'print(1)' 2>/dev/null | tr -d '\r')" = "1" ]; then
    PYTHON="$candidate"
    break
  fi
done
...
# Non-UTF-8 Windows locales (e.g. cp949) break emoji output from the Python helpers
...
create_subagent() {
  ...
  # The PUT is asynchronous (202); a concurrent in-flight update returns 400, so retry.
  local http_code=""
