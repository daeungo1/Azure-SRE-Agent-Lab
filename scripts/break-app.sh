...
HEALTH_STATUS=$(curl -s -o /dev/null -w "%{http_code}" "${APP_URL}/api/fooditems" 2>/dev/null || echo "000")
...
FINAL_STATUS=$(curl -s -o /dev/null -w "%{http_code}" "${APP_URL}/api/fooditems" 2>/dev/null || echo "000")
echo "  Food items API: HTTP ${FINAL_STATUS}"
CART_STATUS=$(curl -s -o /dev/null -w "%{http_code}" "${APP_URL}/api/cart/demo-user" 2>/dev/null || echo "000")
